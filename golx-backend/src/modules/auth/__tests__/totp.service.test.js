...
function buildRepo({ userSecret, deviceSecret } = {}) {
    return {
        ...
        findActiveTotpDevices: jest.fn().mockResolvedValue(
            deviceSecret
                ? [{ id: 'device-1', secret: deviceSecret }]
                : [],
        ),
...
function buildRevokeRepo(devices) {
    return {
        ...
        setPrimaryTotpDevice: jest.fn().mockResolvedValue({ id: 'replacement-device' }),
    };
}
...
describe('TotpService', () => {
    ...
    test('promotes another active device when removing the primary device', async () => {
        const repo = buildRevokeRepo([
            { id: 'primary-device', device_name: 'Old phone', status: 'active', is_primary: true },
            { id: 'replacement-device', device_name: 'New phone', status: 'active', is_primary: false },
        ]);
        const service = new TotpService(repo);

        await expect(service.revokeDevice('user-1', 'primary-device')).resolves.toMatchObject({
            id: 'primary-device',
            status: 'revoked',
            isPrimary: false,
        });
        expect(repo.revokeTotpDevice).toHaveBeenCalledWith('user-1', 'primary-device');
        expect(repo.setPrimaryTotpDevice).toHaveBeenCalledWith('user-1', 'replacement-device');
    });

    test('keeps the last active device from being removed', async () => {
        const repo = buildRevokeRepo([
            { id: 'primary-device', device_name: 'Only phone', status: 'active', is_primary: true },
        ]);
        const service = new TotpService(repo);

        await expect(service.revokeDevice('user-1', 'primary-device')).rejects.toThrow(
            'At least one active MFA device is required',
        );
