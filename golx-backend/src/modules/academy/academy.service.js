...
class AcademyService {
    ...
    async getGroups(branchId, academyId, pagination, actor = null) {
        ...
        if (!branch || (academyId && branch.academy_id !== academyId)) throw new NotFoundError('Branch', branchId);
        if (actor?.role === 'coach') {
            return this.repo.findGroupsForCoachUser(actor.userId, academyId, { ...pagination, branchId });
        }
    ...
    async createGroup(academyId, data) {
        ...
        const birthYearIds = assignmentMode === 'players' && !data.birthYearIds?.length && !data.birthYearId && !data.labels?.length
            ? []
            : await this._resolveGroupBirthYearIds(data.branchId, data);
        const birthYears = await this.repo.findBirthYearsByIds(data.branchId, birthYearIds);
        const normalizedLabels = [...new Set(birthYears.map((row) => row.normalized_label).filter(Boolean))];

        const group = await this.repo.db.transaction(async (trx) => {
            const [created] = await trx('academy_groups').insert({
    ...
    async updateGroup(id, academyId, data) {
        ...
        if (!branch || (academyId && branch.academy_id !== academyId)) throw new NotFoundError('Group', id);
        ...
        const shouldUpdateBirthYears = data.birthYearIds !== undefined || data.birthYearId !== undefined || data.labels !== undefined;
        ...
        const shouldUpdatePlayers = data.playerIds !== undefined || data.playerCodeFrom !== undefined || data.playerCodeTo !== undefined;
        const playerIds = shouldUpdatePlayers ? await this._resolveGroupPlayerIds(group.branch_id, data) : null;
    ...
    async deleteGroup(id, academyId) {
        ...
        if (hasRelations) {
            throw new BadRequestError('Cannot delete a group with active players, sessions, or matches');
        }
    ...
    async getBirthYears(branchId, academyId, actor = null) {
        ...
        if (!branch || (academyId && branch.academy_id !== academyId)) throw new NotFoundError('Branch', branchId);
        if (actor?.role === 'coach') {
            const scopedBranches = await this.repo.findBranchesForCoachUser(actor.userId, academyId, { limit: 500 });
    ...
    async getBirthYearDetail(id, academyId, actor = null) {
        ...
        if (actor?.role === 'coach') {
            const scopedBranches = await this.repo.findBranchesForCoachUser(actor.userId, academyId, { limit: 500 });
            if (!scopedBranches.data.some((item) => item.id === detail.birthYear.branch_id)) {
                throw new ForbiddenError('Coach can only access assigned branches');
            }
        }
    ...
    async createBirthYear(data, academyId) {
        ...
        if (overlap) {
            throw new ConflictError('Birth year range overlaps an existing range in this branch');
        }
    ...
    async updateBirthYear(id, data, academyId) {
        ...
        if (!branch || (academyId && branch.academy_id !== academyId)) throw new NotFoundError('BirthYear', id);
    ...
    async deleteBirthYear(id, academyId, data = {}) {
        ...
        if (affectedPlayers.length) {
            if (!data.transferBirthYearId) {
                throw new BadRequestError('This birth year has players. Transfer them to another birth year before deleting.');
            }
            if (data.transferBirthYearId === id) {
                throw new BadRequestError('Transfer birth year must be different from the deleted birth year');
            }

            const targetBirthYear = await this.repo.findBirthYearById(data.transferBirthYearId);
            if (!targetBirthYear || targetBirthYear.branch_id !== birthYear.branch_id) {
                throw new BadRequestError('Transfer birth year must exist in the same branch');
            }
            ...
            if (!movedBirthYears.length) {
                throw new BadRequestError('Players must have valid birth dates before transfer');
            }

            const nextFromYear = Math.min(targetBirthYear.from_year, ...movedBirthYears);
            ...
            if (overlap) {
                throw new ConflictError('Transfer would make the target birth year overlap another active range');
            }

            await this.repo.db.transaction(async (trx) => {
                await this.repo.updateBirthYearRange(targetBirthYear.id, {
        ...
        if (hasRelations) {
            throw new BadRequestError('Cannot delete birth year label with active players, sessions, or matches');
        }
    ...
    async _resolveGroupBirthYearIds(branchId, data) {
        ...
        if (requestedIds.length) {
            ...
            if (rows.length !== ids.length) {
                throw new BadRequestError('All group birth years must exist in the selected branch');
            }
            return ids;
        }

        const rawLabels = Array.isArray(data.labels) ? data.labels : [];
        const normalizedLabels = [...new Set(rawLabels.map(normalizeLabel).filter(Boolean))];
        if (!normalizedLabels.length) {
            throw new BadRequestError('At least one birth year is required for a group');
        }

        const rows = await this.repo.findBirthYearsByLabels(branchId, normalizedLabels);
        if (!rows.length || new Set(rows.map((row) => row.normalized_label)).size !== normalizedLabels.length) {
            throw new BadRequestError('One or more labels do not exist for this branch');
        }
        return rows.map((row) => row.id);
    }

    async _resolveGroupPlayerIds(branchId, data) {
        const requestedIds = [...new Set(Array.isArray(data.playerIds) ? data.playerIds : [])];
        const rangeRows = data.playerCodeFrom && data.playerCodeTo
            ? await this.repo.findPlayersByCodeRange(branchId, data.playerCodeFrom, data.playerCodeTo)
            : [];
        const ids = [...new Set([...requestedIds, ...rangeRows.map((row) => row.id)])];

        if (!ids.length) return [];

        const rows = await this.repo.findPlayersByIds(branchId, ids);
        if (rows.length !== ids.length) {
            throw new BadRequestError('All group players must exist in the selected branch');
        }
        if ((data.playerCodeFrom || data.playerCodeTo) && !rangeRows.length) {
            throw new BadRequestError('No players were found in the selected player ID range');
        }
        return ids;
    }
