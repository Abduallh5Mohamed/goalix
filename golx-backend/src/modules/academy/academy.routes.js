...
function academyRoutes(controller) {
    ...
    // Branches
    router.get('/branches', rbac('manage_teams'), controller.getBranches);
    router.post('/branches', rbac('manage_teams'), validate({ body: createBranchSchema }), controller.createBranch);
    router.get('/branches/:id', rbac('manage_teams'), validate({ params: uuidParam }), controller.getBranch);
    ...
    // Groups by branch
    router.get('/branches/:id/groups', rbac('manage_teams'), validate({ params: uuidParam }), controller.getGroupsByBranch);

    // Groups
    router.get('/groups', rbac('manage_teams'), validate({ query: groupsQuerySchema }), controller.getGroups);
    ...
    // Birth Years
    router.get('/birth-years', rbac('manage_teams'), validate({ query: birthYearsQuerySchema }), controller.getBirthYears);
    router.post('/birth-years', rbac('manage_teams'), validate({ body: createBirthYearSchema }), controller.createBirthYear);
    router.get('/birth-years/:id', rbac('manage_teams'), validate({ params: uuidParam }), controller.getBirthYear);
