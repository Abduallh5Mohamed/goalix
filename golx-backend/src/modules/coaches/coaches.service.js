...
class CoachesService {
    ...
    async _resolvePlayerAssignmentTargets(coachId, academyId, data = {}) {
        const targetType = data.targetType === 'birth_year' ? 'birth_year' : 'group';
        const groups = await this.repo.findCoachGroupsDetailed(coachId, academyId);

        if (targetType === 'group') {
            const requested = [...new Set((data.groupIds || []).filter(Boolean))];
            if (!requested.length) {
                throw new BadRequestError('Select at least one target group.');
            }
            const allowed = new Set(groups.map((group) => group.id));
            const invalid = requested.find((groupId) => !allowed.has(groupId));
            if (invalid) throw new NotFoundError('Group', invalid);
            return { targetType, groupIds: requested, birthYearIds: [] };
        }

        const requestedBirthYears = [...new Set((data.birthYearIds || []).filter(Boolean))];
        if (!requestedBirthYears.length) {
            throw new BadRequestError('Select at least one target birthday.');
        }
        const allowedBirthYears = new Set(
            groups.flatMap((group) => (group.birth_years || []).map((birthYear) => birthYear.id).filter(Boolean)),
        );
        const invalidBirthYear = requestedBirthYears.find((birthYearId) => !allowedBirthYears.has(birthYearId));
        if (invalidBirthYear) throw new NotFoundError('Birth year', invalidBirthYear);

        const requestedBirthYearSet = new Set(requestedBirthYears);
        const groupIds = [
            ...new Set(
                groups
                    .filter((group) =>
                        (group.birth_years || []).some((birthYear) => requestedBirthYearSet.has(birthYear.id)),
                    )
                    .map((group) => group.id),
            ),
        ];
        if (!groupIds.length) {
            throw new BadRequestError('Selected birthday is not linked to an available group.');
        }
        return { targetType, groupIds, birthYearIds: requestedBirthYears };
    }
    ...
    async createMyPlayerAssignment(userId, academyId, data) {
        ...
        const assignment = await this.repo.createPlayerAssignment({
            ...
            status: 'active',
        }, targets.groupIds, targets.birthYearIds);
    ...
    async updateMyPlayerAssignment(userId, academyId, assignmentId, data) {
        ...
        const targets = shouldUpdateTargets
            ? await this._resolvePlayerAssignmentTargets(coach.id, academyId, {
        ...
        const now = this._assignmentNowFloor();
        const existingOpenAt = existing.open_at ? new Date(existing.open_at) : null;
        const existingDueAt = existing.due_at ? new Date(existing.due_at) : null;
        ...
        if (
            ...
            throw new BadRequestError('Open date cannot be changed after the assignment opens.');
        }
        if (isExistingOpen && dueAt && dueAt < now) {
            throw new BadRequestError('Due date must be now or later for an open assignment.');
        }
        ...
        }, targets?.groupIds ?? null, targets?.birthYearIds ?? null);
    ...
    async reviewMyPlayerAssignmentSubmission(userId, academyId, assignmentId, submissionId, data) {
        const coach = await this._getCurrentCoach(userId, academyId);
        const assignment = await this.repo.findCoachPlayerAssignmentById(assignmentId, coach.id, academyId);
        if (!assignment) throw new NotFoundError('Player assignment', assignmentId);
        await this._assertCoachPermission(coach, academyId, 'can_manage_player_assignments', {
        ...
        const updated = await this.repo.reviewPlayerAssignmentSubmission(submissionId, {
            status: data.status,
            comment: data.comment,
            coachId: coach.id,
            userId,
        });
        return this._shapePlayerAssignmentSubmission({
            ...submission,
            ...updated,
