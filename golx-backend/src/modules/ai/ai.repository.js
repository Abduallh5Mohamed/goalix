...
class AiRepository extends BaseRepository {
    ...
    async _paginateAiScores(query, { page = 1, limit = 20 } = {}) {
        const [{ count }] = await query.clone().clearSelect().count('ai_analyses.id as count');
    ...
    async getAiScores(academyId, pagination = {}) {
        return this._paginateAiScores(this._baseAiScoresQuery(academyId), pagination);
    }

    async getAiScoresForCoach(academyId, coachId, pagination = {}) {
        const query = this._baseAiScoresQuery(academyId)
            .where((scope) => {
                scope
                    .whereIn('player_profiles.id', this._coachAssignedPlayerIdsQuery(coachId))
                    .orWhereExists((exists) => {
                        exists
                            ...
                            .whereRaw('EXTRACT(YEAR FROM player_profiles.date_of_birth)::int BETWEEN aby.from_year AND aby.to_year')
    ...
    async getAiScoresForPlayer(academyId, userId, pagination = {}) {
        return this._paginateAiScores(
            this._baseAiScoresQuery(academyId).where('player_profiles.user_id', userId),
            pagination,
        );
    }
    ...
    async getAiScoresForParent(academyId, parentUserId, pagination = {}) {
        const query = this._baseAiScoresQuery(academyId)
            .where((scope) => {
                scope
                    .whereIn('player_profiles.id', this._parentLinkedPlayerIdsQuery(parentUserId))
                    .orWhereIn('player_profiles.id', this._legacyParentLinkedPlayerIdsQuery(parentUserId));
            });
    ...
    async findParentLinkedPlayer(parentUserId, playerId, academyId) {
        ...
        return this.db('auth_users as au')
            ...
            .whereNull('pp.deleted_at')
            .select('pp.*', this.db.raw('au.id as parent_user_id'), this.db.raw('true as can_view_progress'))
            .first();
    }
    ...
    _parentLinkedPlayerIdsQuery(parentUserId) {
        return this.db('parent_player_links as ppl')
            ...
            .whereNull('ppl.deleted_at')
            .where((scope) => {
                scope.where('ppl.can_view_progress', true).orWhereNull('ppl.can_view_progress');
            })
