...
class AiController {
    ...
    getPerformanceScore = async (req, res, next) => {
        try {
            const score = await this.service.getPerformanceScore(req.params.id, req.user.academyId, req.user);
    ...
    calculatePerformanceScore = async (req, res, next) => {
        try {
            const result = await this.service.calculatePerformanceScore(req.body.playerId, req.user.academyId, req.user);
    ...
    assessInjuryRisk = async (req, res, next) => {
        try {
            const result = await this.service.assessInjuryRisk(req.body.playerId, req.user.academyId, req.user);
            res.status(202).json(ApiResponse.success(result));
        } catch (err) { next(err); }
    };
    ...
    getInjuryRisk = async (req, res, next) => {
        try {
            const risk = await this.service.getInjuryRisk(req.params.id, req.user.academyId, req.user);
    ...
    generateNutritionPlan = async (req, res, next) => {
        try {
            const result = await this.service.generateNutritionPlan(req.body.playerId, req.user.academyId, req.body, req.user);
            res.status(202).json(ApiResponse.success(result));
        } catch (err) { next(err); }
    };
    ...
    getNutritionPlan = async (req, res, next) => {
        try {
            const plan = await this.service.getNutritionPlan(req.params.id, req.user.academyId, req.user);
    ...
    getNutritionPlanHistory = async (req, res, next) => {
        try {
            const plans = await this.service.getNutritionPlanHistory(req.params.id, req.user.academyId, req.user);
