...
class AiController {
    ...
    getPerformanceScore = async (req, res, next) => {
        try {
            const score = await this.service.getPerformanceScore(req.params.id, req.user.academyId);
    ...
    calculatePerformanceScore = async (req, res, next) => {
        try {
            const result = await this.service.calculatePerformanceScore(req.body.playerId, req.user.academyId);
    ...
    assessInjuryRisk = async (req, res, next) => {
        try {
            const result = await this.service.assessInjuryRisk(req.body.playerId, req.user.academyId);
            res.status(202).json(ApiResponse.success(result));
        } catch (err) { next(err); }
    };
    ...
    getInjuryRisk = async (req, res, next) => {
        try {
            const risk = await this.service.getInjuryRisk(req.params.id, req.user.academyId);
    ...
    getInjuryRiskHistory = async (req, res, next) => {
        try {
            const risks = await this.service.getInjuryRiskHistory(req.params.id, req.user.academyId);
    ...
    generateNutritionPlan = async (req, res, next) => {
        try {
            const result = await this.service.generateNutritionPlan(req.body.playerId, req.user.academyId, req.body);
            res.status(202).json(ApiResponse.success(result));
        } catch (err) { next(err); }
    };
    ...
    getNutritionPlan = async (req, res, next) => {
        try {
            const plan = await this.service.getNutritionPlan(req.params.id, req.user.academyId);
    ...
    getNutritionPlanHistory = async (req, res, next) => {
        try {
            const plans = await this.service.getNutritionPlanHistory(req.params.id, req.user.academyId);
    ...
    chat = async (req, res, next) => {
        try {
            const result = await this.service.chat(req.user.userId, req.body.prompt, req.body.context);
            res.status(202).json(ApiResponse.success(result));
        } catch (err) { next(err); }
    };
}
